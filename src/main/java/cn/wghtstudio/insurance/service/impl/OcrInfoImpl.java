...
class PolicyDealImpl implements Runnable {
    ...
    public PolicyDealImpl(String filename, byte[] file, PolicyRepository policyRepository, DrivingLicenseRepository drivingLicenseRepository) {
...
public class OcrInfoImpl implements OcrInfoService {
    ...
    public void policyRecordService(MultipartFile file) throws IOException {
        ...
        if (splitNames[splitNames.length - 1].equals("pdf")) {
            ...
            executorService.submit(new PolicyDealImpl(OSSPath, file.getBytes(), policyRepository, drivingLicenseRepository));
            return;
        }
        ...
        if (splitNames[splitNames.length - 1].equals("zip")) {
            ...
            for (Map.Entry<String, byte[]> entry : entrySet) {
                executorService.submit(new PolicyDealImpl(entry.getKey(), entry.getValue(), policyRepository, drivingLicenseRepository));
            }
